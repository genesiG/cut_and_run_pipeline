...
"""
step_7m_check_collinearity_occupancy.py — Submit bsub job that checks collinearity,
VIF, and standardized beta sensitivity across Unweighted OLS, Response-only IVW, and
Combined IVW specifically for target occupancy / enrichment regressions.
"""
...
R_SCRIPT = os.path.join(os.path.dirname(__file__), "utils", "check_collinearity_occupancy.R")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Submit bsub job: collinearity & VIF diagnostics for occupancy (step_7m)."
    )
...
def run_direct(args):
    ...
    print(f"\n==============================================================")
    print("  step_7m_check_collinearity_occupancy: running directly")
    print(f"==============================================================\n")
    ret = os.system(r_cmd)
    if ret != 0:
...
def run_bsub(args):
    ...
    job_name = f"check_collinearity_occ_{args.bin_size}bp_{region_stem}"
...
"""
    ...
    print(f"\nSubmitting step_7m_check_collinearity_occupancy job: {job_name}")
